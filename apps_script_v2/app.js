...
// ============================================================
const API_URL = 'https://script.google.com/macros/s/AKfycbw4MQfxXzWw70HJqg_XZgO14IbRi-mvhnN_6oZpE6h5WHSsgvTzzBoN8bpSd4q9qLvt/exec';
  ...
  app.innerHTML = `
    ...
    <div class="card">
      ...
      ${data.plano && data.plano.dataUrl
        ? '<img class="plano" src="' + data.plano.dataUrl + '" alt="Plano del tablero ' + escapeHtml(data.nombre) + '" onclick="abrirModalPlano(this.src)">' +
          '<p class="ampliar-hint">🔍 Tocá el plano para ampliarlo</p>'
        : '<div class="sinplano">No hay plano cargado para este tablero.</div>'}
      ${data.observaciones && data.observaciones.trim()
        ? '<div class="observaciones"><span class="observaciones-titulo">📝 Observaciones</span><div class="observaciones-texto">' + escapeHtml(data.observaciones) + '</div></div>'
        : ''}
...
// ------- Visor ampliado del plano (lightbox) -------
function abrirModalPlano(src) {
...
function cerrarModalPlano(evt) {
  ...
  if (img) img.src = ''; // libera memoria de la imagen base64
}

document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') cerrarModalPlano();
});
